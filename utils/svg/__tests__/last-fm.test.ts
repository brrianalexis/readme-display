...
import { type Artist, type LastFmWidgetNowPlaying, THEME_NAMES } from "@/types";
...
describe("last-fm svg utils", () => {
	describe("createSoundBars", () => {
		it.each(
			THEME_NAMES,
		)("should generate correct number of bars for %s theme", (theme) => {
			const bars = createSoundBars(theme);
			const barCount = (bars.match(/<div class="bar/g) || []).length;

			expect(barCount).toBe(SVG_CONFIG.lastfm.now_playing.animation.bars.count);
		});

		it.each(
			THEME_NAMES,
		)("should apply theme class to bars for %s theme", (theme) => {
			const bars = createSoundBars(theme);
	...
	describe("createNowPlaying", () => {
		...
		it.each(
			THEME_NAMES,
		)("should create now playing widget for %s theme", (theme) => {
	...
	describe("createLastFmTopArtistsSVG", () => {
		...
		it.each(
			THEME_NAMES,
		)("should create top artists SVG for %s theme", (theme) => {
			...
			expect(svg).toContain(`height="${SVG_CONFIG.lastfm.top_artists.height}"`);
